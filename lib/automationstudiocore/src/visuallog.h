...
class AUTOMATIONSTUDIO_CORE_EXPORT VisualLog{
...
public:
    ...
    class AUTOMATIONSTUDIO_CORE_EXPORT SourceLocation{
    public:
    ...
    class AUTOMATIONSTUDIO_CORE_EXPORT MessageInfo{
    ...
    class AUTOMATIONSTUDIO_CORE_EXPORT Transport{
    public:
        virtual ~Transport(){}
...
#ifndef vlog
#define vlog(...) as::VisualLog(__VA_ARGS__).at(__FILE__, __LINE__, __FUNCTION__)
#endif // vlog

#ifndef vlog_debug
#ifdef VLOG_DEBUG_BUILD
#define vlog_debug(_configuration, _message) as::VisualLog(_configuration).at(__FILE__, __LINE__, __FUNCTION__).v() << (_message)
#else
