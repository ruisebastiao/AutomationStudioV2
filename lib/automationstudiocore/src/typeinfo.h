...
class AUTOMATIONSTUDIO_CORE_EXPORT TypeInfo{
...
public:
    ...
    void log(as::VisualLog& vl, const QObject* object);
    ...
    void serialize(const QObject* object, as::MLNode& node);
    void deserialize(const as::MLNode& node, QObject* object);
...
private:
    ...
    std::function<void(const QObject*, as::MLNode& node)>  m_serialize;
    std::function<void(const as::MLNode& node, QObject*)>  m_deserialize;
    std::function<void(as::VisualLog& vl, const QObject*)> m_log;
};

template<typename T>
void TypeInfo::addLogging(){
    m_log = [](as::VisualLog& vl, const QObject* obj){
    ...
    std::function<void(const MLNode&, T&)> deserialize)
{
    if ( serialize && deserialize ){
        m_serialize = [serialize](const QObject* obj, as::MLNode& node){
            const T* objtype = qobject_cast<const T*>(obj);
            serialize(*objtype, node);
        };
        m_deserialize = [deserialize](const as::MLNode& node, QObject* obj){
