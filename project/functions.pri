...
defineTest(linkLibrary){
    ...
    LIB_NAME = $$2
    LIB_INCLUDE_PATH = $$PROJECT_ROOT/lib/$$1/include
    !isEmpty(AUTOMATIONSTUDIO_DEV_PATH):LIB_INCLUDE_PATH = $$AUTOMATIONSTUDIO_DEV_PATH/lib/$$1/include
    !isEmpty($$3):LIB_INCLUDE_PATH=$$3
...
#
#   - If this file was linked to from a plugin (AUTOMATIONSTUDIO_BIN_PATH is setup), then:
#       - On windows, if we're building Live CV together with the plugin, then the path is in
#         AUTOMATIONSTUDIO_BIN_PATH/dev/<plugin_path/lib, otherwise the libraries have been deployed in the dev dir
#       - On other systems it's pretty straight forward, AUTOMATIONSTUDIO_BIN_PATH/plugins/<plugin_path> is the actual location
#         of the libraries
#
...
defineReplace(pluginLibraryDeployPath){
    isEmpty(AUTOMATIONSTUDIO_BIN_PATH){ # File is not included from a plugin
        win32:return($$LIBRARY_DEPLOY_PATH/dev/plugins/$$1/lib)
        else:return($$LIBRARY_DEPLOY_PATH)
    } else {
        isEmpty(AUTOMATIONSTUDIO_DEV_PATH){
            error(AUTOMATIONSTUDIO_BIN_PATH setup without AUTOMATIONSTUDIO_DEV_PATH. Both are required from a plugin.)
        }
        win32{ # On windows, we have a separate location for the libraries if we are building from source
            exists($$AUTOMATIONSTUDIO_DEV_PATH/plugins/$$1/lib): return($$AUTOMATIONSTUDIO_DEV_PATH/plugins/$$1/lib)
            else: return($$AUTOMATIONSTUDIO_BIN_PATH/dev/plugins/$$1/lib)
        } else {
            return($$AUTOMATIONSTUDIO_BIN_PATH/plugins/$$1)
        }
    }
}
...
defineReplace(libraryIncludePath){
    isEmpty(AUTOMATIONSTUDIO_DEV_PATH):return($$PROJECT_ROOT)
    else:return($$AUTOMATIONSTUDIO_DEV_PATH)
}
